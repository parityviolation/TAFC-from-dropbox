...
const int stimSetInd[] =  {0,1,2,5,6,7,1};
...
const boolean bRepeatTrialOnPreMature = 1;
   // Set to 1 to make the animal repeat the same stimulus on a PreMature trial
const boolean bEnableCorrectionLoop = 1;
     // Set to 1 to ENABLE correction loops 
const int clThre = 4;
                        // Threshold to activate correction loops, exclusive
const int clCredit = 1;
                      // Decrement in error counter for each correct (and incorrect) trial (but never goes negative)

const int timeValvL = 60;
                    // Calibrate valves for 2.58 ul with 50
const int timeValvR = 60;
                // Calibrate valves for 3.66 ul with 50
...
// Cond Parameters (Designed for Cond of Light Stimulation)
 int ninitialTrialsNoStimulation = 40;
...
// Stimulation  properties
 float condamp[] = {45,35,25};          // percentage of Vcc (usually 5V) NOTE: ramp rate is not changed by condamp
 int condpulsefreq[] = {25,-1,-1};      // use 25Hz se -1 if no pulse
 int condpulsewidth[] = {20,20,20}; 
 int onDelay[]  = {0,0,0};
 int conddur_rampUp[] = {1,1,1};
    // ms ramp rate: define as the time ramp from 0 to PWM = 255 (not by condamp) 
 int onDuration[]  = {3000,3000,3000};
 // time in ms NOTE: Duration time includes dur_rampUp time
 int conddur_rampDn[] = {1,1,1};
    // ms ramp rate: define as the time ramp from PWM = 255 to 0  
// EITHER (Triggered Stimulation)
 ...
 int offDelay[]  = {0,0,0};
 int offStateTrigger[]  = {-1,-1,-1};
                        // This can be any state number, -1 means this trigger will not be used
...
//       Interval stimulation also does use condBlock since it is not tied to the trial structure of a task
 ...
 long stimInterval = 10000 ;
      // this is max interval for random interval time in ms (set to -1 if using onStateTrigger)
 long minstimInterval = 1000;
     // used for randome interval
 boolean brandstimInterval = 0 ;
  // time in ms (set to -1 if using onStateTrigger)
...
int stayCin = 1;
//1 for version of task where the animal has to wait in the center poke for the whole duration of the stimulus
          ...
          if (indexInSessionPM < nPM)          // PM trials still exist so use one and then get rid of it by setting to -1
          {
            stimIndex = sessionstim[indexInSessionPM]; 
             if (!bintervalStimulation){
              ...
              condThisTrial[thisCondIndex] = 0 ;
// not relavent to interval Stimulation
            }
...
// not relavent to interval Stimulation
  ...
  breDraw = 1;
// by default always reDraw (Premature may set this to 0 during this trial)
}
void task()
{
    ...
    if (stimCurrent < 100)  // TROUBLE SHOOTING
      {
         ...
         if (stimCurrent > stimSet[7])  // TROUBLE SHOOTING
      {
         Serial.println(String(-1) + '\t' + String(stimCurrent)+ '\t' +  String(bsessionPM) + '\t' + String(indexInSessionPM) + '\t' +  String(indexInSession) + '\t' + String(sessionstim[indexInSession])  + '\t' + String(stimSet[stimIndex]) + '\t' + String(sessioncond[indexInSession]) + '\t' + String(long(condamp[thisCondIndex])) + '\t' +  String(onStateTrigger[thisCondIndex]));
       }
  ...
  case 38: // premature_left_poke_in state
    ...
    else if(bRepeatTrialOnPreMature)
    {
        breDraw =0;
    }
  ...
  case 40: // choice_miss state
    ...
    case 78: // premature_stayC_IN AKA broke_fixation state
    digitalWrite(ledC,HIGH);
