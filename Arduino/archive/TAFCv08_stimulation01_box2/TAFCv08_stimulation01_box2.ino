...
const boolean bredrawOnPM = 0;                    // SET TO 1 for performance Set to 0 for training when you don't want a new stimulus after premature
const int durerrorTone = 150; 

const int timeValvL = 90; // Calibrate valves for 2.8 ul with 75
const int timeValvR = 75; // Calibrate valves for 2.834 ul with 60
// PnOut CHECK PIN_OUT FOR VALVES
const int syncLed = 2;
const int spkr = 3;
const int pkC = 8;
const int ledC = 9;
...
const int ninitialTrialsNoStimulation = 40;
const int condstimPin[] = {22,22,22};                         // output pin to signal that laser is on
const int condaoPin[] = {aoPIN,aoPIN,aoPIN};                       // output pin supporting PWM
// Stimulation Block properties
const boolean brandomizeBlock = 1 ;                // set to 0 for stimulation in blocks 
const int condblockSize =10;                  
const int ncond = 1;
const int  sizeInCondBlock[] = {0,0,0};
// Stimulation  properties
const float condamp[] = {45,0,0};          // percentage of Vcc (usually 5V) NOTE: ramp rate is not changed by condamp
const int condpulsefreq[] = {25,-1,-1};      // use -1 if no pulse
const int condpulsewidth[] = {20,20,20}; 
const int onDelay[]  = {0,0,0};
const int conddur_rampUp[] = {1,1,1};     // ms ramp rate: define as the time ramp from 0 to PWM = 255 (not by condamp) 
const int onDuration[]  = {4000,0,0};  // time in ms NOTE: Duration time includes dur_rampUp time
const int conddur_rampDn[] = {1,1,1};     // ms ramp rate: define as the time ramp from PWM = 255 to 0  
// EITHER (Triggered Stimulation)
const int onStateTrigger[]  = {56,-1,-1};                           // This can be any state number, -1 means this trigger will not be used
const int offDelay[]  = {0,0,0};
const int offStateTrigger[]  = {-1,-1,-1};                         // This can be any state number, -1 means this trigger will not be used
...
const long stimInterval = 10000 ;       // this is max interval for random interval time in ms (set to -1 if using onStateTrigger)
const long minstimInterval = 1000;      // used for randome interval
...
void updateStimCond()
{  
  if (trialNum < ninitialTrialsNoStimulation)
  {
    if (breDraw)
    {
    ...
    indexInSession = -1; // this shouldn't be necessaray but some how indexInSession can be -1 
  }
  else
  {
  ...
  breDraw = 1; // by default always reDraw (Premature may set this to 0 during this trial)
}
void task()
{
  ...
  case 23: // wait_Cin state
    ...
    if (stimCurrent < 100) // TROUBLE SHOOTING
      {
  ...
  case 38: // premature_left_poke_in state
    ...
    Serial.println(strToPrint);
     ...
     if  (bredrawOnPM & (trialNum > ninitialTrialsNoStimulation))
    {
    ...
    {
      breDraw = 0; // This insure that next trial will be the same cond and stimulus as this trial
    }
