...
// Task parameters
int scaling = 3000; // (ms)
double initSet[] = {0.2, 0.35, 0.42, 0.46, 0.54, 0.58, 0.65, 0.8};
int stimSet[sizeof(initSet)/sizeof(double)];
int bound = scaling/2;
int ITI = 9000;
int timeOut = 20000;
int choiceDeadline = 5000;

int timeValvL = 110; // Calibrate valves for 25 ul box1
int timeValvR = 175; // Calibrate valves for 25 ul box1

//int timeValvL = 192; // Calibrate valves for 50 ul
//int timeValvR = 275; // Calibrate valves for 50 ul
...
void loop() {
  ...
  switch (state) {
    ...
    case 22: // state_0 state
     ...
     digitalWrite(syncLed,HIGH);
     trialNum++;
     if(drawStim){stimIndex = random(sizeof(stimSet)/sizeof(int));} // Argument of random() should be length(stimSet). ToDo: figure out how to implement length() in Arduino's language
    ...
    case 26: // correct_Lin state
     ...
     if(error[stimIndex]<0){error[stimIndex] = 0;}
     if(error[stimIndex]>clThre) { drawStim = false;}
     else {drawStim = true;}
     state = 30;
     strToPrint = String(state) + '\t' + String(millis());
     Serial.println(strToPrint);
