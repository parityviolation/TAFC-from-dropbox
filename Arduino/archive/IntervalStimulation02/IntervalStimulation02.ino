...
int onDelay[]  = {
  0,0,0};
int onDuration[]  = {
  4000,4000,4000};     // time in ms  //NOTE: duration includes the ramp up (but not the ramp down)
const boolean bintervalStimulation = 1;
const unsigned long stimInterval =25000 ; // this is max interval for random interval time in ms (set to -1 if using onStateTrigger)
const boolean brandstimInterval = 1; // time in ms (set to -1 if using onStateTrigger)
const unsigned long minstimInterval = 15000; // used for random interval
//int onStateTrigger[]  = {
//  -1,-1,-1};       
int offDelay[]  = {
  0,0,0};
...
float condamp[] = {
  85,0,0};                       // percentage of Vcc (usually 5V) NOTE: this doesn't change ramp rate
