...
//const int stimSetInd[] =  {0,1,2,5,6,7};
...
const int durerrorTone = 150; 
const int preTrialStateDur = 1500;
const int initTrialMaxLatency = 4000;
long rampOnDur = 0; //variable that defines duration of ramp up and ramp ON,NOT INCLUDING RAMP DOWN
long ITI = 0;
long ITIafterStim = 500; //short period garanteed to have no stimulation before a new trial can be initiated
...
const int clThre = 4;                         // Threshold to activate correction loops, exclusive
const int clMax =  10;                        // number of correct answers to escape correction loop
...
const int aoPIN = 44;                        //TIMER 5 (REMEBER TO SET TIMER)

// Cond Parameters (Designed for Cond of Light Stimulation)
 int ninitialTrialsNoStimulation = 0;

 
 int condstimPin[] = {22,22,22};                         // output pin to signal that laser is on
...
// Stimulation Block properties
 ...
 const int ncond = 1;
 int  sizeInCondBlock[] = {2,0,0};
// Stimulation  properties
 float condamp[] = {17,35,25};          // percentage of Vcc (usually 5V) NOTE: ramp rate is not changed by condamp
 int condpulsefreq[] = {7,-1,-1};      // use 25Hz se -1 if no pulse
 int condpulsewidth[] = {10,20,20}; 
 int onDelay[]  = {0,0,0};              //delay from onStateTrigger 
 int conddur_rampUp[] = {1,1,1};     // ms ramp rate: define as the time ramp from 0 to PWM = 255 (not by condamp) 
 int onDuration[]  = {0,0,0};  // time in ms NOTE: Duration time includes dur_rampUp time
 int conddur_rampDn[] = {1,1,1};     // ms ramp rate: define as the time ramp from PWM = 255 to 0  
// EITHER (Triggered Stimulation)
 int onStateTrigger[]  = {87,56,56};                           // This can be any state number, -1 means this trigger will not be used
 int offDelay[]  = {0,0,0};
...
unsigned long state87On;
unsigned long state23On;
int trialNum = 0;
int stimCurrent;
int stimIndex;
...
rampOnDur = preTrialStateDur + initTrialMaxLatency + scaling + choiceDeadline; //variable that defines duration of ramp up and ramp ON,NOT INCLUDING RAMP DOWN
onDuration[0]  = rampOnDur; //setting up rampOnDur
...
{  
  ...
  {// TROUBLESHOOT    Serial.println(String(indexInSession) + '\t' +  String(stimIndex));// BA
    Serial.println(String(bsessionPM) + '\t' +  String(indexInSession) + '\t' + String(sessionstim[indexInSession])  + '\t' + String(stimSet[stimIndex]) + '\t' + String(sessioncond[indexInSession]) + '\t' + String(long(condamp[thisCondIndex])) + '\t' + String(onStateTrigger[thisCondIndex])+ '\t' + String(onDuration[thisCondIndex])+ '\t' + String(thisCondIndex));
  }
...
void task()
{
  ...
  case 87: // wait trail avail state (ramp up state in stimulated trials)

    if((millis()-state87On) > preTrialStateDur)
    {
  ...
  case 23: // wait_Cin state
    ...
    if((millis()-state23On) > initTrialMaxLatency)
      {
        ...
        digitalWrite(ledC,HIGH);
        swTrialOn = millis();
        ITI = scaling + choiceDeadline + conddur_rampDn[0]+ ITIafterStim;  //this variable has to be change if we stop using the first index of the array for stimulation conditions 
        repeatStimCondHelper();                                            // Since this condition 
      ...
      ITI = scaling + choiceDeadline + conddur_rampDn[0]+ ITIafterStim + (initTrialMaxLatency-(millis()-state23On));  //this variable has to be change if we stop using the first index of the array for stimulation conditions 
      strToPrint = String(11) + '\t' + String(millis());
      Serial.println(strToPrint);
      digitalWrite(ledC,HIGH);
      swTrialOn = millis();
