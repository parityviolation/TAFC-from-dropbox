...
const int timeValvL = 120;      //BASS 1/9/2014 calibrated by eye :)               // Calibrate valves for 2.58 ul with 50
const int timeValvR = 100;                 // Calibrate valves for 3.66 ul with 50
const boolean bErrorON_Left = 1;                 // set to 0 to let animals keep poking after an 'erroneous' poke on the Left
const boolean bErrorON_Right = 1;                 // set to 0 to let animals keep poking after an 'erroneous' poke on the Right
...
// Cond Parameters (Designed for Cond of Light Stimulation)
 int ninitialTrialsNoStimulation =20;
  ...
  case 25: // wait_Sin state
    if((millis()-swStimOff)>choiceDeadline){
      ...
      if(stimLong){
        ...
        else if(valuePkR&bErrorON_Right){
          state = 27;
          strToPrint = String(state) + '\t' + String(millis());
          Serial.println(strToPrint);
        }
        ...
        else if(valuePkR&bErrorON_Left){
          state = 28;
          strToPrint = String(state) + '\t' + String(millis());
          Serial.println(strToPrint);
        }
