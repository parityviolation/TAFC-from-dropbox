...
int onDelay[]  = {
  0,0,0};
int onDuration[]  = {
  15000,4000,4000};     // time in ms  //NOTE: duration includes the ramp up (but not the ramp down)
const boolean bintervalStimulation = 1;
const unsigned long stimInterval = 65000 ; // this is max interval for random interval time in ms (set to -1 if using onStateTrigger)
const boolean brandstimInterval = 1; // time in ms (set to -1 if using onStateTrigger)
const unsigned long minstimInterval =50000; // used for random interval
const int superblocksize = 25;
...
{// Turn on Stimulation Pin
  if( timeonDelay != -1 && onDelay[thisCondIndex] != -1 && (millis() - timeonDelay ) > onDelay[thisCondIndex]){
    ...
    strToPrint = String(300) + '\t' + String(millis()) + '\t' + String(indexStimulationInBlock) + '\t' + String(thisCondIndex) + '\t' + String(int(condamp[thisCondIndex])) + '\t' + String(onDuration[thisCondIndex]) + '\t' + String(condpulsefreq[thisCondIndex])  +'\t' + String(condpulsewidth[thisCondIndex]);
