...
// Stimulation Block properties
 ...
 const int ncond = 1;
 int  sizeInCondBlock[] = {5,0,0};
// Stimulation  properties
 float condamp[] = {20,35,25};          // percentage of Vcc (usually 5V) NOTE: ramp rate is not changed by condamp
 int condpulsefreq[] = {-1,-1,-1};      // use 25Hz se -1 if no pulse
 int condpulsewidth[] = {10,20,20}; 
 int onDelay[]  = {0,0,0};              //delay from onStateTrigger 
 int conddur_rampUp[] = {1000,1,1};     // ms ramp rate: define as the time ramp from 0 to PWM = 255 (not by condamp) 
 int onDuration[]  = {0,0,0};  // time in ms NOTE: Duration time includes dur_rampUp time
 int conddur_rampDn[] = {1000,1,1};     // ms ramp rate: define as the time ramp from PWM = 255 to 0  
...
ITI = rampOnDur + conddur_rampDn[0]+ ITIafterStim;  //this variable has to be change if we stop using the first index of the array for stimulation conditions 
