...
int timeValvL = 70; // Calibrate valves for 
int timeValvR = 70; // Calibrate valves for 
...
int clSaturation = 4; // The maximum number of sequential correct trials required to escape a correction loop
...
void loop() {
  ...
  switch (state) {
    ...
    case 26: // correct_Lin state
     ...
     if(error[stimIndex] == 0 && drawStim == false) { drawStim = true;}
     state = 30;
     strToPrint = String(state) + '\t' + String(millis());
     Serial.println(strToPrint);
     break;
    ...
    case 27: // error_Rin state
     ...
     if(error[stimIndex]>clThre) { 
       drawStim = false; 
       if (error[stimIndex]>clSaturation)
           {error[stimIndex]=clSaturation;}
         }
     state = 34;
     strToPrint = String(state) + '\t' + String(millis());
     Serial.println(strToPrint);
