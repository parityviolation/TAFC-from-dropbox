...
boolean bdebug =0;
int selectStart=-1; // set to 0 to start on LEft block 1 for right 
                       // -1 for random

// Task parameters
int a = 1;
int pHi =  100; // 0-100% Higher reward probability
int pLo =  0; // 0-100% Lower reward probability
int blockLenMin = 50;
int blockLenMax = 50; // this + blockLenMin
int ITI = 5000; // (ms)
int choiceDeadline = 4000; // (ms)
int timeOut = 6000; // (ms)
int rwdDelay = 0; // (ms)
int timeValvL = 55; // 60
int timeValvR = 65; // 60
int waitTarget = 0;// Time (ms) the animal is required to wait at the center poke
int waitMin = 0;
int wait = waitMin;
int waitIncr = 0;
int waitDecr = 0;
...
int condProbOfStimulation[] = {30,30,30} ;   // number between 0 and 100} NOTE for blocks only all number will be set to 100 automatically
int  sizeInCondBlock[] = {
  1,1,1};
// Stimulation  properties
float condamp[] = {
  100,75,75};   //53 for a867     //17 for 1013  // (30 for SERT) 39 10mW  20for 5mW 17 for 1.5mW percentage of Vcc (usually 5V) NOTE: ramp rate is not changed by condamp
//int condpulsefreq[] = {40,25,12};     //45 5.5mW // use 25Hz se -1 if no pulse
int condpulsefreq[] = {
  10,12,6};     //45 5.5mW // use 25Hz se -1 if no pulse
int condpulsewidth[] = {
  20,20,20}; 
int onDelay[]  = {
  0,0,0};
...
int onDuration[]  = {
  2000,1000,1000};  // time in ms NOTE: Duration time includes dur_rampUp time
int conddur_rampDn[] = {
  1,1,1};     // ms ramp rate: define as the time ramp from PWM = 255 to 0  
// EITHER (Triggered Stimulation)
int onStateTrigger[]  = {
 28,28,28};                           // This can be any state number, -1 means this trigger will not be used
//  26,26,26};                           // This can be any state number, -1 means this trigger will not be used
int onStateTrigger1[]  = {
 27,27,27};                           // This can be any state number, -1 means this trigger will not be used
//  26,26,26};                           // This can be any state number, -1 means this trigger will not be used
int onStateTrigger2[]  = {
 26,26,26};                           // This can be any state number, -1 means this trigger will not be used
//  29,29,29};                           // This can be any state number, -1 means this trigger will not be used
int onStateTrigger3[]  = {
 29,29,29};                           // This can be any state number, -1 means this trigger will not be used
//  29,29,29};                           // This can be any state number, -1 means this trigger will not be used
int offDelay[]  = {
...
void setupStimulation()
{
  ...
  if (btestlaser){stimulationSelector = -1;}
...
void stimulationTriggerHelper()
{
  ...
  if (thisCondIndex!= -1)
  {
    if(condPIN_Status[thisCondIndex] == 0 && (state == onStateTrigger[thisCondIndex]||state == onStateTrigger1[thisCondIndex]||state == onStateTrigger2[thisCondIndex]||state == onStateTrigger3[thisCondIndex]) )
    { 
      if (bdebug){
          strToPrint = String(-2000) + '\t' + String(condProbOfStimulation[thisCondIndex])  + '\t' + String(onStateTrigger[thisCondIndex])+ '\t' + String(onStateTrigger1[thisCondIndex]);
      Serial.println(strToPrint);
      }
