function getBonsaiExtremes2(dpArray,bonsaiParams)%%( bstruct,bonsaiFileName)
...
for idp = 1: length(dpArray)
    ...
    if processTime <  30 % check process time
        warning(['Bonsai took:' num2str(processTime(idp)) 's to process ' dpArray(idp).FileName]);
    else
        disp(['Bonsai took:' num2str(processTime(idp)) 's to process ' dpArray(idp).FileName]);
    end
