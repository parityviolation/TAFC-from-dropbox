...
framesBefore = timeBefore*round(dp.video.info.meanFrameRate);
timeAfter = 3; %time after align condition in seconds
framesAfter = timeAfter*round(dp.video.info.meanFrameRate);
...
bonsaiFileName = fullfile(rd.Dir.BonsaiCode, 'clip_recorder.bonsai');
...
%to rename the files - MouseName_Protocol_Date_SSAB_stim20_correct_0-Clip1
%- example
...
for i = 1:dpValid.ntrials
    ...
    listFilesNew{i,:} = [fname '_stim' num2str(stim(i)) '_correct_' num2str(correct(i)) '_laser_' num2str(laser,'%d') '-Clip' num2str(counter(ind)) '.avi'];
end
