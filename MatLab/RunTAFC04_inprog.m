...
questions(n).askIfBox = [1:6] ;% if question 1 is 0
n = 3;
...
questions(n).askIfAnimal = {};
n= 7;
questions(n).text = 'reward changed (y/n)';
questions(n).fieldname = 'rewardChanged';
questions(n).answer = [];
questions(n).answerAllowEmpty = 1;
questions(n).answerType = 'bool';
questions(n).askIf = []; % if question 4 is 1
...
questions(n).askIfAnimal = {};
n= 8;
questions(n).text = 'Previous reward size';
questions(n).fieldname = 'previousReward';
questions(n).answer = [];
questions(n).answerAllowEmpty = 0;
questions(n).answerType = 'string';
questions(n).askIf = [7 1]; % if question 4 is 1
questions(n).askIfBox = [] ;
questions(n).askIfAnimal = {};
n= 9;
questions(n).text = 'New reward size:';
questions(n).fieldname = 'newReward';
questions(n).answer = [];
questions(n).answerAllowEmpty = 0;
questions(n).answerType = 'string';
questions(n).askIf = [7 1]; % if question 4 is 1
...
bonsaiFile = {[BonsaiCodePath '\BonsaiV2\video_acq_pg_timestamped_ledv1_GPIO_box1.bonsai'];
    ...
    [BonsaiCodePath '\BonsaiV2\video_acq_pg_timestamped_ledv1_GPIO_box5.bonsai'];
    [BonsaiCodePath '\BonsaiV2\video_acq_pg_timestamped_ledv1_GPIO_box6.bonsai'];
    [BonsaiCodePath '\OpenField_PG_centroid2.bonsai']};
...
%% which protocol
protocolList = {'TAFCv06_8stim';'TAFCv06_2stim';'TAFCv06_4stim';'TAFCv06_6stim';'TAFCl1'; 'TAFCl2';'TAFCl3';'TAFCl3_more_increment';'Flush_Valves';'TAFCv07';'TAFCv06_8stim_longerTimeout';'MATCHINGvFix02_Stimulation1';'TAFCv08_stimulation03';'TAFCv09';'SelfReinf01';'SelfReinf01_water';'TAFCv08_Fixation_stimulation02';'MATCHINGvFix02_Stimulation_Blocks1';'IntervalStimulation03';'TAFCv08_stimulationREV03';'CRF';'TAFCl3_BULK';'TAFCl3_BULK_reward_prob';'TAFCl3_BULK_reward_prob_tone';'BULK_big_small_tone_fix';''};
protocolIndex = input(['\nWhich task protocol do you want to load on Arduino? \n' ...
    ...
    '\n12. TAFCv08_stimulation03 - with simulation code\n'...
    ...
    '\n17. MATCHINGvFix02_Stimulation_Blocks1\n'...
    '\n18. IntervalStimulation\n'...
    '\n19. TAFCv08_stimulationREV03 - with simulation code and reversed pokes\n'...
    '\n20. CRF for arch mistery, with center poke\n'...
    '\n21. TAFCl3 FOR BULK \n'...
    '\n22. TAFCl3 FOR BULK With variable reward \n'...
    '\n23. TAFCl3 FOR BULK With variable reward and tone predicting it \n'...
    '\n24. BULK big reward and small reward with fixation and tone predicting \n'... 
    '\n25. skip upload\n\n'],'s');
...
if(brunVideo &     ~bdebug)
    ...
    fileGPIO = fullfile(fvdatadilr,[fn '_gpio.csv']);
    ...
    if bonsaiFile{boxIndex}
        if   boxIndex < 7 & boxIndex~=2 & boxIndex~=4 & boxIndex~=1 & boxIndex~=5 & boxIndex~=6 & boxIndex~=3
            changeBonsaiFiles(bonsaiFile{boxIndex}, fileAvi,fileAviTime,fileLED,fileTracking,fileCentroidXY,fileCentroidOrientation);
        end
        
        if (boxIndex==1)||(boxIndex==2)||(boxIndex==3)||(boxIndex==4) ||(boxIndex==5)||(boxIndex==6)% run NEW NEW bonsai
...
%              C:\Software\BonsaiRC4
        ...
        elseif 0 (boxIndex==1) % run NEW bonsai
            camIndex = rd.bbox.cameraIndex;
...
% if ~isempty(data.rewardChanged ), data.comment = [data.comment ' ' 'Rew was' data.previousReward 'NOW' data.newReward ]; end
