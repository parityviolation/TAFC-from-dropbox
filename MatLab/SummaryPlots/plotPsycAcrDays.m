function dpArray =  plotPsycAcrDays(varargin)
...
IntervalSet = dpArray(1).IntervalSet;
 ...
 %IntervalSet = [0.2000 0.3500 0.4200 0.5800 0.6500 0.8000];
%  IntervalSet = [0.2000 0.3500 0.4200 0.4600 0.5400 0.5800 0.6500 0.8000];

% if bconc
%     dpArray = concdp(dpArray);
%     nAnimal = 'nAnimal_Conc';
% end
...
for ifile = 1:length(dpArray);
    ...
    for icond = 1:2 % collect the data from each condition and file into a matrix
...
hfig.hf = figure ('WindowStyle','docked','NumberTitle','off','Name',savename);
plotAnn(sAnnot)
...
for ifld = 1:length(plotdesc.fld)
    ...
    for icond = 1:2
        ...
        hl = errorbar(hAx,IntervalSet',y',semy',cond(icond).color)
...
if bsave
    orient tall
    saveas(hfig.hf , fullfile(r.Dir.SummaryFig, [savename '.pdf']));
end
