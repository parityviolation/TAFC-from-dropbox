...
XL = [min(dpC.IntervalSet) max(dpC.IntervalSet)]*dpC.Scaling/1000 + [-1 1]*0.05*dpC.Scaling/1000; % xrange to plot
...
if bplot
    ...
    for ihypn = 1 : num_hypothesis_to_test
        ...
        for icond = 1 : number_fits
            ...
            if this_dpC.ntrials
                if bploterrorbar % this probably shouldn't be in this function
                    ...
                    if bplotRawdata
                        try
                            h.hle = errorbar(hAx,this_dpC.IntervalSet*this_dpC.Scaling/1000,y,this_dpC.fracLongCI(1,:)-y,this_dpC.fracLongCI(2,:)-y,'color',cond(icond).color );
                            hold on;
                    ...
                    line(this_dpC.stats.IntervalSet*this_dpC.Scaling/1000,this_dpC.stats.frac.choiceLong,'linestyle','none','Marker','.','color',cond(icond).color,'parent',hAx);hold all
                end
                fx = [0:0.001:1];
                x = [0:dpC.Scaling/1000/(length(fx)-1):dpC.Scaling/1000];
        ...
        if 0 % plot all intervals
            set(hAx,'XTick',dpC.IntervalSet*dpC.Scaling/1000,'XTickLabel',cellfun(@(x) num2str(x,' %1.1f'),num2cell(dpC.IntervalSet*dpC.Scaling/1000),'UniformOutput',false))
            rotateXLabels(gca,40);
        else % plot first, last and boundry
            xticklabels = cellfun(@(x) num2str(x,' %1.1f'),num2cell(dpC.IntervalSet*dpC.Scaling/1000),'UniformOutput',false);
            xticklabels{end+1} = xticklabels{end};
            for i = 2: length(xticklabels)-1, xticklabels{i} = ''; end
            xticklabels{length(dpC.IntervalSet)/2+1} = num2str(dpC.Scaling/1000/2);
            
            set(hAx,'XTick',sort([dpC.IntervalSet 0.5]*dpC.Scaling/1000) ,'XTickLabel',xticklabels)
        end
