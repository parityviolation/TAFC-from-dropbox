function [fs1, fs2, er] = comp_struct(s1,s2,n1,n2,p,tol, bdisplay)
% function [fs1, fs2, er] = comp_struct(s1,s2,n1,n2,p,tol, bdisplay)
...
if nargin < 3 || isempty(n1);  n1 = 's1'; end
if nargin < 4 || isempty(n2); n2 = 's2'; end
if nargin < 5 || isempty(p); p = 0; elseif p ~= 1 && p ~= 0; p = 0; end
if nargin < 6 || isempty(tol); tol = 1e-6; end
if nargin < 7 || isempty(bdisplay); bdisplay = 1; end
    ...
    for ii=1:numel(pnt1i)
        ...
        for jj = 1:size(s1,2)
            ...
            [fss1 fss2 err] = comp_struct(getfield(s1(jj),char(fn1(pnt1i(ii)))), ...
                getfield(s2(jj),char(fn2(pnt2i(ii)))),n1p,n2p,p,[],bdisplay);
...
elseif isstruct(s1) ~= isstruct(s2)
    ...
    if p; if bdisplay; disp('Paused .....'); pause;  end ; end
    ...
    if isequal(s1,s2);
        ...
        if strcmp(num2str(size(s1)),num2str(size(s2)));
            ...
            if ischar(s1) || iscell(s1) || ...
                ...
                if ischar(s1)
                    er1 = sprintf('%s is char - %s',n1,char(s1)); if bdisplay; disp(er1); end
                end
                if ischar(s2)
                    er2 = sprintf('%s is char - %s',n2,char(s2)); if bdisplay;  disp(er2); end;
                end
...
if 1
    if ~isempty(fs1)
        for ii = 1:length(fs1)
            if strcmp(n1,fs1(ii))
                if bdisplay; disp(sprintf('First Structure non-matched fields:	 %s',[n1])); end
            else
    ...
    if ~isempty(fs2)
        for ii = 1:length(fs2)
            if strcmp(n2,fs2(ii))
                if bdisplay ; disp(sprintf('Second Structure non-matched fields:	 %s',[n2]));end
            else
