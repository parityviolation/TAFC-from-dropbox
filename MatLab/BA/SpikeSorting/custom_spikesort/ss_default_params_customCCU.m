function spikes = ss_default_params_customCCU(Fs, varargin )
...
%
    ...
    spikes.params.thresh =5;               % for 'auto', set number of standard deviations above background noise
 %  spikes.params.thresh =    [-5.3897 -5.0999 -5.1816 -5.5274]; % for 'manual', set specific thresholds for each channel
    spikes.params.window_size  = 2;       % ms, width of a spike
%     spikes.params.window_size  = 1.3;       % ms, width of a spike
%     spikes.params.shadow = 0.75;            % ms, enforced dead region after each spike
    spikes.params.shadow = 3.5;            % ms, enforced dead region after each spike
%     spikes.params.cross_time = 0.4;         % ms, alignment point for peak of waveform
    spikes.params.cross_time = 0.6;         % ms, alignment point for peak of waveform
 
    % sorting parameters
    spikes.params.refractory_period = 2.0;  % ms, refractory period (for calculation refractory period violations)
