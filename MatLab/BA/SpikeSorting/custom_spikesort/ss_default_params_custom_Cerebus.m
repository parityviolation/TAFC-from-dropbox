...
function spikes = ss_default_params_custom_Cerebus(Fs,spikes, varargin )
...
%
    ...
    spikes.params.detect_method = 'manual';  % 'auto' = threshold calculated from background noise, 'manual' = user defined threshold
 ...
 %  spikes.params.thresh =    [-5.3897 -5.0999 -5.1816 -5.5274]; % for 'manual', set specific thresholds for each channel
    spikes.params.window_size  = 48/Fs*1000;       % ms, width of a spike
    spikes.params.shadow = 1;            % ms, enforced dead region after each spike
    spikes.params.cross_time = 10/Fs*1000;         % ms, alignment point for peak of waveform
