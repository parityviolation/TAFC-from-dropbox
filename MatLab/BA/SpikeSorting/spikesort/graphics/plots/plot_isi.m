function hax = plot_isi( spikes, show, show_isi,hax )
...
%
    ...
    % check arguments
    if nargin < 3|isempty(show_isi), show_isi = spikes.params.display.show_isi;  end
    if nargin < 2|isempty(show), show = 'all'; end
    ...
    set( hax,'UserData', data,'Tag','isi' )

    % write updating method
    update_isi( [], [], show_isi, hax);
end
...
function hbb = make_isi     
    ...
    if data.show_isi
       ...
       ylabel({['No. of spikes ' num2str(length(spiketimes),'%d')],data.ystr})
        ...
        hold on, hbb = bar(lags*1000,cross,1.0); hold off;  
        set(hbb,'FaceColor',[0 0 0 ],'EdgeColor',[0 0 0 ],'Tag','line')
