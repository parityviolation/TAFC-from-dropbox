function spikes = ss_kmeans_custom(spikes, options)
...
% BA for memory saving
if bpackspikes 
    save('tempspikes','spikes'); clear spikes;
end
% BA for memory saving END
...
assigns = ones(M, opts.reps,CLASS); 
...
for rep = 1:opts.reps                                 % TOTAL # REPETITIONS
	...
	for iter = 1:opts.divisions                       % # K-MEANS SPLITS
		...
		while (~(assign_converge | mse_converge))     % convergence?
		    ...
		    clustdistsq = single(pairdist(waves, centroid, 'nosqrt'));  % BA added to save memory
            [junk,assigns(:,rep)] = min(clustdistsq,[],2); clear junk;  % BA added to save memory
...
% BA for memory saving
clear clustdistsq bestdists oldassigns wavelist
if bpackspikes
    load('tempspikes','spikes')
end
% BA for memory saving END
