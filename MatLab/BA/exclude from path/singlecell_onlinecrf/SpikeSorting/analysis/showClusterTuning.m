...
if ~isfield(expt.sort.defaults,'plotTuning_fileInd') || bChooseFile
    ...
    save( fullfile(rigdef.Dir.Expt,getFilename(expt.info.exptfile)),'expt'); % BA this seems like it should be changed to save at rig specific location
...
spikes = filtspikes(spikes,0,'assigns',selected_assigns,'fileInd',selectedfileInd);

% plotSortTuning(expt,tempspikes,selectedfileInd);

if exist('bLed','var')
plotclusterTuning(expt,spikes,selectedfileInd,bLed);
else
plotclusterTuning(expt,spikes,selectedfileInd);
end
