...
ind = regexp(tempfilename,'\.');
if ~isempty(ind), tempfilename = tempfilename(1:ind-1); end
...
switch typ
    case 'nidaq'
        ...
        s = sprintf('Loading daq file... %s',LOADFILE);
        disp(s);        
...
%             data = data*1000;
    ...
    if isfield(STF.tdt,'epocrange')
                    [data dt] = loadTDTData(STF,chns,STF.tdt.epocrange);
