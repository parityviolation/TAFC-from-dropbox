...
for i = 1:length(expt.files.names)
    ...
    % Add expt.files.varparam1 and expt.files.varparam2 (this allow access
    % to stimulus block in struct desribing files)
    expt.files.varparam1{i} = varparam(1).Name;
    if length(varparam) == 2
        expt.files.varparam2{i} = varparam(2).Name;
    elseif length(varparam) == 1
        expt.files.varparam2{i} = NaN;
    end
end