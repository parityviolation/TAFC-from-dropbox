function spikes = spikesAddSweeps(spikes,expt,trodeInd,fileInds)
% function spikes = spikesAddSweeps(spikes,expt,trodeInd,fileInds)
%
...
%   7/24/10 - BA added optional fileInds input,this allows the code to be used
%    independent from the trodes in expt structure
%    Note: if fileInds is specified, trodeInd input is ignored
%

if nargin>3 & ~isempty(fileInds)
    if ~isempty(trodeInd), warning('trodeInd will be ignored, since fileInds have been specified'); end
else
