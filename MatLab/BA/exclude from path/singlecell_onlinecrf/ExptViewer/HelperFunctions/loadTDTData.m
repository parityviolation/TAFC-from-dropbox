...
loadTDThelper_getStimCond(blkPATH ,EPOCNAME);
...
%     figure(1); hist(diff(allEpocs(2,:)),20);title('hist of time between sweeps') ; xlabel('time (s)');
    ...
    if sweeplength > temp;
        sprintf('************************** sweeplength cannot exceed time to next Epoc: %1.3f ****************',temp)
        sweeplength = temp;
...
%     allEpocs = invoke (TT, 'GetValidTimeRangesV');                              % Gets the start and end of the Time ranges.
    ...
    if length(epocrange)==1; % get just one epoc                                                   %  getting just 1 sweep
        TT.SetGlobalV('T2',allEpocs(2,epocrange(1))+sweeplength+dt*2);
    else
        TT.SetGlobalV('T2',allEpocs(2,epocrange(2)));
    end
end
