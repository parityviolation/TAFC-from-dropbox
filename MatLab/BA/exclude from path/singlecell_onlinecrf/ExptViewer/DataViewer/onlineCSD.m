function h = onlineCSD(dvHandles)
%
% INPUT
%   dvHandles: DataViewer handles
%
% OUTPUT
%   h: guidata for the CSD figure
%
%   Created: BA 6/5/10
...
% BA ADD check that all channels are enabled
...
h.window = [0 h.sweeplength];
h.Fs = dvHandles.SampleRate;
h.windowPts = round(h.window*h.Fs );
h.xdata = h.window(1):1/h.Fs:h.window(2);

% Make figure
h.lfpFig = figure('Visible','off','Color',[1 1 1], ...
    'Position',rigdef.onlineLFP.Position,'Name','CSD','NumberTitle','off');
...
% Make axes
margins = [0.02 0.15 0.042 0.035];
axPos = dvAxesPosition(1,margins);
...
% Make CSD
        ...
        h.lines(m,n) = imagesc('Parent',h.axs(m),'Color',tempColor,'Visible','off', ...
