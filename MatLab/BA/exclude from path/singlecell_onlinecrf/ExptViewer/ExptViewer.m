...
catch ME % This error may occur if importing old expt struct (should create a function to update old expt structs
    getReport(ME)
end
...
if isequal(scluster,'ReCluster')
    ...
    % Cluster spikes
    sprintf('Aligning and Clustering %d spikes\t trode %d, chns: %s',length(spikes.spiketimes),spikes.info.trodeInd,num2str(handles.expt.sort.trode(spikes.info.trodeInd).channels));
    ...
    % Set sort flag
    handles.expt.sort.trode(spikes.info.trodeInd).sort = 'incomplete';
...
function MergeChk_Callback(hObject, eventdata, handles)
...
for i = 1:numUnits
    ...
    unit(i).spikespersec = [];
    unit(i).peakrate = [];
    unit(i).spikewaveform.amplitude = [];             % Need function that pulls out these paramters from raw data
    unit(i).spikewaveform.width = [];
    unit(i).spikewaveform.peak = [];
    unit(i).spikewaveform.trough = [];
    unit(i).spikewaveform.troughpeakratio = [];
    unit(i).spikewaveform.maxampchannel = [];
    unit(i).spikewaveform.waveformtype = [];
    unit(i).spikewaveform.avgwave = ComputeAvgWaveform(spikes,unitAssigns(i));
    unit(i).avgwaveform = ComputeAvgWaveform(spikes,unitAssigns(i));
end
    ...
    for i = 1:length(trode)  % Loop on trodes
        for j = 1:size(trode(i).unit,2)  % Loop on units
            if ~isempty(trode(i).spikesfile)
                ...
                UnitTable{unitNum,7+1} = trode(i).unit(j).spikewaveform.waveformtype;
                UnitTable{unitNum,8+1} = num2str(trode(i).unit(j).spikewaveform.amplitude);
...
% Open spikes file shown in SorFileFileUp
trodeList = get(handles.SortFilePopUp,'String');
Selection = get(handles.SortFilePopUp,'Value');
if Selection > length(trodeList)
    Selection = length(trodeList);
...
% handles.expt.sort.unittags = unitList;
...
function UnitTable_CreateFcn(hObject, eventdata, handles)
