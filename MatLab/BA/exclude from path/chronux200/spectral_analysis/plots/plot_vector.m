...
if nargin < 4 || isempty(Xerr);
    ...
    if length(Xerr)==1;
        h = plot(f,X);
        h2 = line(get(gca,'xlim'),[Xerr,Xerr],'Color',c,'LineStyle','-','Linewidth',w);
        h = [h h2];
    elseif ~isempty(Xerr);
        h = plot(f,X);
        set(h,'Color',c);
        hold on; h2 = plot(f,Xerr(1,:),'Linewidth',w/3); set(h2,'Color',c); h3 = plot(f,Xerr(2,:),'Linewidth',w/3); set(h3,'Color',c);% BA modified to have solid line with 3rd of width
        h = [h h2 h3];
    end
end
xlabel('f');
