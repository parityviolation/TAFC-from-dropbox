function [img, ii] = plaidimage( g, npix, aperture )
...
%
% img = plaidimage(g, npix, 'circle')
% makes a circular aperture default: 'none'
%
% [img, ii] = plaidimage...
%
% EXAMPLE
...
% 2001 Matteo Carandini created
% 2007-10 MC made it deal with NaN values of ori and sf
% 2008-12 LB added an option for a circular aperture
...
ii = zeros(npix,npix);
for igrat = 1:ngrats
    if g(igrat).c > 0 % if it is zero, the ori or sf might be NaN....
...
switch aperture
    case 'circle'
        dd = sqrt(xxx.^2+yyy.^2);
        %win = double(dd >= sinpars(igrat).dima/2 & dd <= sinpars(igrat).dimb/2);
        win = double(dd <= 1);
    case 'none'
        win = 1;
    otherwise
        error('<plaidimage> Unknown type of aperture (%s)', aperture);
end
ii = ii.*win;

% make a white 'background' (useful if you want to blend it into a
% document)
% comment this out or create an option if you want a gray background
ii(win==0) = 1;
