function h = plotAligned_spikes(spikes,alignEvent,WOIin,ElectUnit,options)
% function h = plotAligned(spikes,alignEvent,ElectUnit,options)
%
...
savefile = sprintf('%s_%s_E%d_U%d%s%s ',alignEvent, sDate,Electrode,UnitID );
sAnn = savefile;
...
if ~isfield(options,'BOOTHSTRAP')
    options.BOOTHSTRAP = 0;
end
...
clear h
sVisibility = 'off';
h.fig = figure('Position',[   1114          40         554         942],'Name',savefile, 'NumberTitle','off','Visible',sVisibility);
...
for iInterval = 1:nInterval
    ...
    options.hAx = axes();hold(options.hAx ,'on');
