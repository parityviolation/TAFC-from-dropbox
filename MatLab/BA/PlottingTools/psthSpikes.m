...
% 2014 BE AWARE:   WOI now is NOT all positive 
bverbose = 0;
alpha = 0.05;
...
if ~isempty(spikes)
    ...
    if ~isempty(spiketimes)
        ...
        sem =  nanstd(n_trial/binsize)/sqrt(numtrials);
        nci(1,:) = n + sem;
        nci(2,:) = n - sem;
    ...
    if NSMOOTH  % BA bootstrap doesn't work with smoothing for now
        ...
        % calculate SEM after smoothing
        n_trial_smooth = nanconv(n_trial,kernel);
        sem =  nanstd(n_trial_smooth/binsize)/sqrt(numtrials);
        nci(1,:) = y + sem;
        nci(2,:) = y - sem;
...
%         $BELOW
        ...
        nci = bootci(nbootstraps,{meanfunc,n_trial_smooth},'alpha',alpha);
...
if isfield(plotparam,'endline')
    if ~isempty(plotparam.endline ) % remove plot after endline
        ...
        if bplot
            ...
            [hPsthDot, hPsthpatchDot] = boundedline(centers(1:end)',dotted_y',zeros(size(dotted_y)),hAxes,'alpha','transparency',0.5);
...
if bplot
    % plot line and boostrapped confidence
    [hPsth, hPsthpatch] = boundedline(centers(1:end)',y',zeros(size(y)),hAxes,'alpha','transparency',0.3);
    ...
    if ~isempty(spiketimes)&&all(~isnan(spiketimes)), axis(hAxes,[WOI(1) WOI(2) 0 maxN]);  end
