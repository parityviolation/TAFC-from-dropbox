...
clear all
trialNum = randi([5 20], [100 6 2]);

for n = 1:100 % neurons
    for s = 1:6 % stimuli
        for d = 1:2 % decisions
            firingRates(n,s,d,:,1:trialNum(n,s,d)) = abs(randn(100,trialNum(n,s,d)) + 5);
            firingRatesAverage(n,s,d,:) = mean(firingRates(n,s,d,:,1:trialNum(n,s,d)), 5);
        end
    end
end
