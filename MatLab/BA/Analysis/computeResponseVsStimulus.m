function varargout = computeResponseVsStimulus(spikes,stim,cond,w,bsubspont)
...
%   bsubspont if 1 subtract mean spontaneous rate from each field (default
%   to 1)
% OUTPUT
...
% Created:  5/16/10 - SRO
%           6/1/11 BA make subtraction of spontaneous rates optional

if ~exist('bsubspont'), bsubspont = 1; end
...
for m = 1:size(cspikes,1)
    for n = 1:size(cspikes,2)
        for i = 1:length(wnames)
            temp = wnames{i};
            [fr.(temp)(m,n) frsem.(temp)(m,n)] = computeFR(cspikes(m,n),w.(temp));
        end
    end
end

% Substract spontaneous rate from each window except spont window
if bsubspont % if specified
    wnames = fieldnames(fr);
    for i = 1:length(wnames)
        temp = wnames{i};
        if ~strcmp(temp,'spont')
            fr.(temp) = fr.(temp) - repmat(nanmean(fr.spont),size( fr.(temp),1),1);
        end
    end
end
