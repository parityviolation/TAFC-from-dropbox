...
% Add led window field
if ~isfield(expt,'led')
    expt = led_def(expt); % overkill ... way of manually adding LED timing
end
...
analysisTypes = {'orientation','spatial','contrast','odors','movieSTA'};
...
function h = getEditVal(h,analysisType)
...
%     unitList = []
%     try unitList = handles.expt.analysis.currentAnalysis.unitTags; end
%     if isempty(unitList)        
        unitList = expt.sort.unittags;
        label = expt.sort.label;
        nspks = expt.sort.nspks;
        for i = 1:length(unitList), s(i) = {sprintf('%s %s %s,',unitList{i},label{i},nspks{i})}; end
        [unitInd, ok] = listdlg('ListString',s,'ListSize',[175 300],...
            'Name','Choose units for analysis');
        unitList = unitList(unitInd);
%     end
      
    varparamName = expt.stimulus(fileInd(1)).varparam(1).Name ;
    if isequal(lower(varparamName),'none')
        varparamName = expt.stimulus(fileInd(1)).params.StimulusName;
    end
    
    % loop throug units
    hFig = []; % intialize unit figure handles holder for 
    for iclus = 1:length(unitList)
        
        switch lower(varparamName)
            case 'orientation'
                h = getEditVal(h,lower(varparamName));
                expt = h.expt;
                hFig{iclus} = orientationFig(expt,unitList{iclus},fileInd,h.chkVal);
                % %                 fr{i} = allPolarPlots(expt,unitList{i},fileInd,h.chkVal);
            case 'contrast'
                h = getEditVal(h,lower(varparamName));
                expt = h.expt;
                hFig{iclus} = contrastFig(expt,unitList{iclus},fileInd,h.chkVal);
            case 'odors'
                hFig{iclus} = odorFig(expt,unitList{iclus},fileInd,h.chkVal);
            case 'movie'
                movieSTAFig(expt,unitList{iclus},fileInd,h.chkVal);
            case 'spatial freq'
                hFig{iclus} = orientationFig(expt,unitList{iclus},fileInd,h.chkVal);
            case 'eyes' % thisis for size tuning
                h = getEditVal(h,'sizetuning');
                expt = h.expt;
                hFig{iclus} = sizetuningFig(expt,unitList{iclus},fileInd,h.chkVal);
                
            otherwise
                display(varparamName);
        end
...
%     if length(unitList)>1 && ~isempty(hFig)
%         summaryUnitFigure(expt,unitList,hFig)
%     end
...
fileInd = [1:length(expt.files.names)];
    fileInd = GUI_listFilesVisualParams('exptStruct',expt,'fileInd',fileInd);
    ok = 1;
if ok
