...
%BVA
ses = fieldnames(myTAFC.SertxChR2_179)

for thissess = fieldnames(myTAFC.SertxChR2_179)'
    if any(isnan(myTAFC.SertxChR2_179.(thissess{1}).neur.Counts(:)))
        myTAFC.SertxChR2_179 = rmfield(myTAFC.SertxChR2_179,thissess{1})
    end
end


Animals = fieldnames(myTAFC);
filename = 'myTAFCm_that8_NANout';
savevar = true;
N = 100;
...
for a = 1:length(Animals)
    ...
    bigSess = [];
    bhv = [];
    myFields = {'ChoiceLeft','ChoiceCorrect','Interval','Premature'};
...
%         bigSess = [bigSess; bhv]
    ...
    for i = Intervals
       ...
       MLE{a,Intervals==i}.choiceP = MLE{a,Intervals==i}.choiceE;
        ...
        clear counts
        %% Sudopops - Premature trials
        counts = [];        
        for s = 1:length(Sessions)
            bhv =  myTAFC.(animal).(Sessions{s}).bhv;
            bhv = prematurePsycurvHelper(bhv);
            bhv.IntervalwithPM = round(bhv.IntervalwithPM*100);
            iNdx = bhv.IntervalwithPM == i*100;
            nanNdx = any(squeeze(any(isnan(myTAFC.(animal).(Sessions{s}).neur.Counts),2)),2)';
            cNdx = bhv.Premature == 1;
            temp = myTAFC.(animal).(Sessions{s}).neur.Counts(iNdx & ~nanNdx & cNdx,:,:);
            if any(iNdx & ~nanNdx & cNdx)
                try
                    counts = cat(3,counts,temp(randsample(size(temp,1),N),:,:));
                catch
                    sudoNdx = [1:size(temp,1), randsample(size(temp,1),N-size(temp,1),true)'];
        ...
        for n = 1:N
            ...
            MLE{a,Intervals==i}.choiceP.max(n,binNdxTe) = Yte.max;
            MLE{a,Intervals==i}.choiceP.cm(n,binNdxTe) = Yte.cm;
            MLE{a,Intervals==i}.choiceP.mat(binNdxTe,binNdxTe,n) = Yte.mat;
