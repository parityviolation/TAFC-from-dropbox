...
setappdata(hObject,'hled1',handles.led1);
setappdata(hObject,'hled2',handles.led2);
setappdata(hObject,'hvstim',handles.vstim);
...
function psthTuneButton_Callback(hObject, eventdata, handles)

if get(hObject,'Value')
    handles.psthTune = onlinePSTHTuning(handles);
    setappdata(handles.hDataViewer,'psthTuneON',1)
    guidata(hObject,handles)
elseif ~get(hObject,'Value')
    setappdata(handles.hDataViewer,'psthTuneON',0)
end
