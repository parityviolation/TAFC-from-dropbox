...
h.window = [0.04 1];
h.Fs = 32000;% dvHandles.SampleRate; % BA 6/5/10 modified used to be hardcoded 32000
...
% Make axes
if isfield(rigdef.onlineLFP,'axesmargins')
    margins = rigdef.onlineLFP.axesmargins;
else
...
for m = 1:size(h.lfpData,1)
    for n = 1:size(h.lfpData,2)
        h.lfpData{m,n} = zeros(size(h.xdata))';
        set(h.lines(m,n),'XData',h.xdata,'YData',h.lfpData{m,n},'Visible','on');
    end
end
% Make lfps visible
lfpOn(h,h.rvOn)
