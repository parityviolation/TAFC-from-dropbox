...
if isempty(expt.probe.xdistance)
    expt.probe.xdistance =getFromExptTable(ExptTable,'Recording depth')
     if isstr(expt.probe.xdistance )
       expt.probe.xdistance  =  str2num(expt.probe.xdistance );  
     end
end

% Temporary for old files
if isnan(expt.probe.type)
    probeList = {'4x8 bundle','2x2','1x16','4x1'};
...
if strcmp(lower(expt.probe.type),'16 channel')
...
elseif strcmp(lower(expt.probe.type),'glass electrode')
    expt.probe.trode.names = {'E1'};                  % Name is user-defined
    expt.probe.numchannels = 1;
    expt.probe.numtrodes = 1;
    expt.probe.sitesPerTrode = 1;
elseif strcmp(lower(expt.probe.type),'32 Bundle')
    expt.probe.trode.names = {'1'};                  % Name is user-defined
    expt.probe.numchannels = 1;
    expt.probe.numtrodes = 1;
    expt.probe.sitesPerTrode = 1;
end
