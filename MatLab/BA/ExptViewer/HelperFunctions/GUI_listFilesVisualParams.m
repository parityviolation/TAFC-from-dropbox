...
expt = handles.expt;
if ~isfield(handles,'fileInd')  % if subset of fileInds not specified use all fileInds
    handles.fileInd = 1:length(expt.files.names);
end
...
for i = 1:length(fileInd)
    varparam = expt.stimulus(fileInd(i)).varparam;
    tabledata{i,1} = expt.files.names{fileInd(i)};
    nvar = length(varparam);
    if ~isempty(regexp(lower(expt.stimulus(fileInd(i)).params.StimulusName),'movie') )% get movie file name is movie
        tabledata{i,2} = expt.stimulus(fileInd(i)).params.StimulusName;
        mvname = getFilename(expt.stimulus(fileInd(i)).params.MovieName);
        tabledata{i,3} =mvname;
        tabledata{i,4} =num2str(expt.stimulus(fileInd(i)).params.MovieRate);
    else
        for j = 1:nvar
            tabledata{i,(j-1)*2+2} = varparam(j).Name;
            tabledata{i,(j-1)*2+3} = num2str(varparam(j).Values,'%1.3g ');
        end
    end
end

try set(handles.VarParamTable,'Data',tabledata); end
