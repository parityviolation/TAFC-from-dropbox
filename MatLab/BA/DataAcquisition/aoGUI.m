...
DEF_duration = AIOBJ.SamplesPerTrigger/AIOBJ.SampleRate;
...
if noaoWavObj
    ...
    set(handles.rampup,'String',0.5);
    set(handles.rampdown,'String',0.5);
    set(handles.Amplitude,'String',5);
    set(handles.VoltageOffset,'String',RigDef.ao.Offset{1});
    set(handles.Freq,'String',.01);
    set(handles.Duration,'String',DEF_duration);
    set(handles.nrepeat,'String',1);
    set(handles.repeatdelay,'String',1);
    
end


% BA This should be replaced with something like the GetGUIvalues call
...
for i = 1:length(RigDef.ao.ID)
    ...
    handles.aoWavObj(i).SampleRate = 3200;
    %     handles.aoWavObj(i).Duration =
    %     AIOBJ.SamplesPerTrigger/AIOBJ.SampleRate;
    % BA hack to set the duration that I want
    handles.aoWavObj(i).Duration = min(AIOBJ.SamplesPerTrigger/AIOBJ.SampleRate,DEF_duration);
    handles.aoWavObj(i).NumSamples =  handles.aoWavObj(i).SampleRate *handles.aoWavObj(i).Duration;
    ...
    handles.aoWavObj(i).CurrentIndex.TimeOffset = 1;
    handles.aoWavObj(i).NumPulses = 1;
    handles.aoWavObj(i).Width = 1.5;
    handles.aoWavObj(i).CurrentIndex.Width = 1;   
    handles.aoWavObj(i).rampup = 0.5;
    handles.aoWavObj(i).CurrentIndex.rampup = 1;
    handles.aoWavObj(i).rampdown = 0.5;
    handles.aoWavObj(i).CurrentIndex.rampdown = 1;
    handles.aoWavObj(i).Amplitude = 5;
    handles.aoWavObj(i).CurrentIndex.Amplitude = 1;
    ...
    % repeats
    handles.aoWavObj(i).nRepeat = 1;
    handles.aoWavObj(i).CurrentIndex.nRepeat = 1;
    
    handles.aoWavObj(i).RepeatDelay = 1;
    handles.aoWavObj(i).CurrentIndex.RepeatDelay = 1;
...
set(handles.Duration,'String',handles.aoWavObj(1).Duration );
...
set(hObject,'Units','Pixels','Visible','on');
...
function rampup_Callback(hObject, eventdata, handles)
handles = GetGuiValues(handles);
DisplayWaveform(handles);
toggleAONotUpdated(handles,1);

function rampdown_Callback(hObject, eventdata, handles)
handles = GetGuiValues(handles);
DisplayWaveform(handles);
toggleAONotUpdated(handles,1);
...
function repeatdelay_Callback(hObject, eventdata, handles)
handles = GetGuiValues(handles);
DisplayWaveform(handles);
toggleAONotUpdated(handles,1);
...
global AIOBJ
for i = 1:length(handles.aoWavObj)
    if 0 % autoupdate ao duration to ai duration
...
switch lower(s)
    case 'pulse train'
        set([handles.Freq handles.txtfreq],'Enable','On','Visible','On')
        set([handles.NumPulses handles.txtpulses],'Enable','On','Visible','On')
        set([handles.rampup handles.txtrampup],'Visible','Off')
        set([handles.rampdown handles.txtrampdown],'Visible','Off')
        
        set(handles.uiprepeat,'Visible','Off')
        if 1/str2num(get(handles.Freq,'String')) <... % fix case where pulse is longer than pulse period
                str2num(get(handles.Width,'String'))
            warning('Pulses in a Train cannot have a Width > 1/Freq')
            set(handles.Width,'String',num2str(1/str2num(get(handles.Freq,'String'))/2))
        end
    case 'square'
        set([handles.Freq handles.txtfreq],'Enable','Off','Visible','Off')
        set([handles.NumPulses handles.txtpulses],'Enable','Off','Visible','Off')
        set([handles.rampup handles.txtrampup],'Visible','Off')
        set([handles.rampdown handles.txtrampdown],'Visible','Off')
        set(handles.uiprepeat,'Visible','On')
        
    case 'trapezoid'
        set([handles.Freq handles.txtfreq],'Enable','Off','Visible','Off')
        set([handles.NumPulses handles.txtpulses],'Enable','Off','Visible','Off')
        set([handles.rampup handles.txtrampup],'Visible','On')
        set([handles.rampdown handles.txtrampdown],'Visible','On')
        set(handles.uiprepeat,'Visible','On')
...
%update Duration and Number of samples
for i = 1:length(handles.aoWavObj)
    handles.aoWavObj(i).Duration = str2num(get(handles.Duration,'String'));
    handles.aoWavObj(i).NumSamples = handles.aoWavObj(i).Duration *handles.aoWavObj(i).SampleRate;
end
i = get(handles.ID,'Value');

% PULSE TRAIN fix case where pulse is longer than pulse period
s = get(handles.Waveform,'String'); s = s{get(handles.Waveform,'Value')};
if isequal(s,'pulse train')
    if 1/str2num(get(handles.Freq,'String')) <...
            str2num(get(handles.Width,'String'))
        warning('Pulses in a Train cannot have a Width > 1/Freq');
        set(handles.Width,'String',num2str(1/str2num(get(handles.Freq,'String'))/2))
    end
...
% Repeat Params
handles.aoWavObj(i).nRepeat = str2num(get(handles.nrepeat,'String'));
handles.aoWavObj(i).RepeatDelay = str2num(get(handles.repeatdelay,'String'));
...
% dont let width b egreat than duration
handles.aoWavObj(i).Width = str2num(get(handles.Width,'String'));
handles.aoWavObj(i).Width = min(handles.aoWavObj(i).Duration, handles.aoWavObj(i).Width);
set(handles.Width,'String',num2str(handles.aoWavObj(i).Width));

% don't let Time Offset + width be greather than duration
if handles.aoWavObj(i).TimeOffset + handles.aoWavObj(i).Width> handles.aoWavObj(i).Duration
    handles.aoWavObj(i).TimeOffset = handles.aoWavObj(i).Duration - handles.aoWavObj(i).Width;
    set(handles.TimeOffset,'String',num2str(handles.aoWavObj(i).TimeOffset));
end
...
% *** for trapezoid mode
% don't allow rampup + offset to be later than duration
if isequal(s,'trapezoid')
    handles.aoWavObj(i).rampup = str2num(get(handles.rampup,'String'));
    temp = handles.aoWavObj(i).rampup +handles.aoWavObj(i).TimeOffset;
    if temp > handles.aoWavObj(i).Duration
        warning( 'offset + rampup  > duration');
    end
    % don't allow offset + rampup + width to be greater than duration
    temp = handles.aoWavObj(i).rampup +handles.aoWavObj(i).TimeOffset+handles.aoWavObj(i).Width;
    if temp > handles.aoWavObj(i).Duration
        warning( 'offset + rampup + width > duration');
    end
    
    handles.aoWavObj(i).rampdown = str2num(get(handles.rampdown,'String'));
    temp = handles.aoWavObj(i).rampup +handles.aoWavObj(i).TimeOffset+handles.aoWavObj(i).Width + handles.aoWavObj(i).rampdown;
    if temp > handles.aoWavObj(i).Duration
        warning( 'offset + rampup + width + rampdown > duration');
    end
end
...
% This code should be replaced by some function that takes care of all Ind
CurrentIndex.TimeOffset = handles.aoWavObj(i).CurrentIndex.TimeOffset;
TimeOffset = handles.aoWavObj(i).TimeOffset(CurrentIndex.TimeOffset);
set(handles.OffsetSlider,'Value',TimeOffset);

CurrentIndex.Amplitude = handles.aoWavObj(i).CurrentIndex.Amplitude;
Amplitude = handles.aoWavObj(i).Amplitude(CurrentIndex.Amplitude);
...
% tempAmplitude = num2str(aoWavObj(i).Amplitude);
% % AmplitudeInd = aoWavObj(i).AmplitudeInd;
...
% Set slider values
tempAmp = str2double(get(handles.Amplitude));
if length(tempAmp)==1,set(handles.AmplitudeSlider,'Value',tempAmp); end % only set if not a series of amplitudes

tempOffset = str2double(get(handles.TimeOffset));
if length(tempOffset)==1, set(handles.OffsetSlider,'Value',tempOffset); end
...
function rampup_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end
...
function rampdown_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end
...
function repeatdelay_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end
