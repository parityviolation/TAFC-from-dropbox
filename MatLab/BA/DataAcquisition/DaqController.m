...
temp = strcmp(temp.ConstraintValue,RigDef.Daq.TriggerMethod);
handles.board.HwDigitalTriggerSource  = RigDef.Daq.HwDigitalTriggerSource ; % define it even if not used


if any(temp) 
    handles.board.TriggerType = RigDef.Daq.TriggerMethod;
else
...
UDP_OBJ_STIM_PC.OutputBufferSize = 2^12;
...
handles.aiparams.StartFcn = {@updateDataViewerName,hObject};
...
if strcmp(AIOBJ.Running,'Off') % only change these when not running
    ...
    AIOBJ.HwDigitalTriggerSource =  handles.board.HwDigitalTriggerSource;  % not only used in if TriggerType is HwDigital
...
set(AIOBJ,'StartFcn',handles.aiparams.StartFcn)
...
elseif status == 0
    ...
    if get(handles.checkbox4_HardStop,'Value') == 1;
        ...
        AIOBJ.SamplesAcquiredFcn= {@Stop,AIOBJ};
