function [dpArray h] = plot_CMByInterval_CorrectVsError(dpArray,condCell,plottype,options)
...
switch plottype
    case 'x'
        ylab = 'pos (cm)';
        savfileheader = [savfileheader '_CMX'];
    case 'y'
        ylab = 'pos (cm)';
        savfileheader = [savfileheader '_CMY'];
    case 'speed'
        ylab = 'speed (cm/s)';
        savfileheader = [savfileheader '_CMSpeed'];
    case 'ExtremeNosex'
        ylab = 'pos (cm)';
        savfileheader = [savfileheader '_ENoseX'];
    case 'ExtremeNosey'
        ylab = 'pos (cm)';
        savfileheader = [savfileheader '_ENoseY'];
    case 'ExtremeNosespeed'
        ylab = 'speed (cm/s)';
        savfileheader = [savfileheader '_ENoseSpeed'];
end
...
for iIntv = 1:length(listIntv)
    ...
    cond(1).baseline = 0.1;
    if bIntervalGTX
        cond(1).filter={'ChoiceCorrect',1,'Interval',@(x) x >= Intv *.999,'stimulationOnCond',0};
    else
        cond(1).filter={'ChoiceCorrect',1,'Interval',Intv,'stimulationOnCond',0};
    end
    ...
    cond(2).desc = 'error';
    if bIntervalGTX
        cond(2).filter = {'ChoiceCorrect',0,'Interval',@(x) x >= Intv,'stimulationOnCond',0};
    else
        cond(2).filter = {'ChoiceCorrect',0,'Interval',Intv,'stimulationOnCond',0};
    end
    ...
    switch plottype
       ...
       case 'ExtremeNosex'
            data =  dpC.video.extremes.xy(:,1); % dpC.video.cm.speed;
            data = data* cm_px ; % cm
        case 'ExtremeNosey'
            data =  dpC.video.extremes.xy(:,2); % dpC.video.cm.speed;
            data = data* cm_px ; % cm
        case 'ExtremeNosespeed'
            data =  dpC.video.extremes.noseSpeed;
            data = data* cm_px *fm_sec; % cm/sec
    end
